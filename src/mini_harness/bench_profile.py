"""Overrides used when MINI_HARNESS_PROFILE=bench.

A benchmark run is unattended, has a fixed time budget, and works on a
throwaway container, so the interactive safety rails get switched off and
the system prompt tells the model nobody is listening.
"""

import os


def wall_budget_from_env(default: float = 1400.0) -> float:
    raw = os.environ.get("MINI_HARNESS_WALL_BUDGET", default)
    try:
        value = float(raw)
...
W3. Change: use edit_file for targeted edits; write_file for new files, or with
    overwrite=true to replace an existing file entirely.
...
D5. Solve the task yourself. Do not search for, download, or copy a published
    ...
    is expected and fine; looking up the answer is not.
